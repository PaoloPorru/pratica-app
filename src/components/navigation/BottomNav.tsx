        ...
        {navItems.map(({ href, icon: Icon, label }) => {
          ...
          return (
            <Link key={href} href={href}
              className={cn("flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl transition-all duration-200",
                isActive ? "text-pratica-green-dark" : "text-pratica-muted")}>
              <div className={cn("w-10 h-7 flex items-center justify-center rounded-xl transition-all duration-200",
                isActive ? "bg-pratica-green-light" : "bg-transparent")}>
                <Icon size={isActive ? 22 : 20} />
              </div>
              <span className={cn("text-[10px] font-medium tracking-wide transition-all",
